...
const NEWS_ARTICLES = [
  { title: 'IP Linux 24.04 LTS Released with New Features', source: 'iplinux.com', time: '2h ago' },
...
const generateSimulatedPage = (url: string): string => {
  ...
  return `
    ...
    <body>
      <div class="header">
        <h1>${host}</h1>
        <p>Simulated page for IP Linux Browser</p>
      </div>
      <div class="content">
        <h2>Welcome to ${host}</h2>
        <p>This is a simulated version of the website running inside IP Linux Browser. Many real websites block iframe embedding for security reasons.</p>
  ...
  return (
    <div className="h-full flex flex-col items-center justify-start pt-12 custom-scrollbar overflow-auto" style={{ background: 'var(--bg-window)' }}>
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 style={{ fontSize: '28px', fontWeight: 700, color: 'var(--text-primary)' }}>IP Linux Browser</h1>
      </div>
...
export default function Browser() {
  ...
  const [bookmarks, setBookmarks] = useState<Bookmark[]>(() => {
    try {
      const saved = localStorage.getItem('iplinux_browser_bookmarks');
  ...
  useEffect(() => {
    localStorage.setItem('iplinux_browser_bookmarks', JSON.stringify(bookmarks));
  }, [bookmarks]);
  ...
  const closeTab = useCallback((tabId: string) => {
    ...
    if (activeTabId === tabId) {
      setTabs((prev) => {
        if (prev.length === 1 && prev[0].id === tabId) return prev; // handled above
        const idx = tabs.findIndex((t) => t.id === tabId);
        const remaining = tabs.filter((t) => t.id !== tabId);
        if (remaining.length > 0) {
          const nextIdx = Math.min(idx, remaining.length - 1);
          setTimeout(() => setActiveTabId(remaining[nextIdx].id), 0);
        }
        return prev;
      });
    }
        ...
        <div className="h-full flex items-center justify-center" style={{ background: 'var(--bg-window)' }}>
          <div className="flex flex-col items-center gap-4">
            <div className="relative w-10 h-10">
              ...
              />
              <Globe size={20} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{ color: 'var(--text-disabled)' }} />
            </div>
            <span style={{ fontSize: '12px', color: 'var(--text-disabled)' }}>Loading page...</span>
          </div>
      ...
      <iframe
        ...
        onLoad={(e) => {
          ...
          try {
            ...
            // Cross-origin — page loaded but we can't access it, which is fine
            setIframeError(false);
          }
        }}
      />
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: 'var(--bg-window)' }}>
      ...
      >
        ...
        <button
          onClick={() => {
            if (activeTab.url !== 'home' && !activeTab.url.startsWith('search://')) {
              window.open(activeTab.url, '_blank');
            }
          }}
          className="flex items-center justify-center rounded-lg transition-all"
          style={{ width: 32, height: 32, opacity: (activeTab.url === 'home' || activeTab.url.startsWith('search://')) ? 0.3 : 1 }}
