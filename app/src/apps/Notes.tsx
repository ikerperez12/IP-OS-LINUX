...
const loadNotes = (): Note[] => {
  try {
    const saved = localStorage.getItem('iplinux_notes');
...
const loadFolders = (): FolderItem[] => {
  try {
    const saved = localStorage.getItem('iplinux_note_folders');
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
...
const Notes: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('iplinux_notes', JSON.stringify(notes));
  }, [notes]);

  useEffect(() => {
    localStorage.setItem('iplinux_note_folders', JSON.stringify(folders));
  }, [folders]);
        ...
        {activeNote ? (
          <>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(activeNote.content, { ADD_ATTR: ['style'] }) }}
            />
