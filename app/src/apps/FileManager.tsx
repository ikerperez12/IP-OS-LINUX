...
const getFileIcon = (name: string, type: FileSystemNode['type']) => {
  ...
  if (name.endsWith('.png') || name.endsWith('.jpg')) return FileText;
  return FileText;
};

const getAppForFile = (name: string) => {
  if (name.endsWith('.txt') || name.endsWith('.js') || name.endsWith('.ts')) return 'texteditor';
  if (name.endsWith('.md')) return 'markdownpreview';
  if (name.endsWith('.json')) return 'jsonformatter';
  if (name.endsWith('.png') || name.endsWith('.jpg')) return 'imageviewer';
  if (name.endsWith('.zip') || name.endsWith('.tar')) return 'archivemanager';
  return 'texteditor';
};
...
export default function FileManager() {
  ...
  const children = currentFolderId ? fs.getChildren(currentFolderId) : [];
