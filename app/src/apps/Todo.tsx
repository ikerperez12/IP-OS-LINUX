...
const loadTasks = (): Task[] => {
  try {
    const saved = localStorage.getItem('iplinux_todos');
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
  const today = new Date().toISOString().split('T')[0];
  return [
    { id: generateId(), title: 'Explore the IP Linux desktop', completed: false, dueDate: today, priority: 'high', tags: ['welcome'], projectId: 'inbox', createdAt: Date.now() },
...
const loadCustomProjects = (): Project[] => {
  try {
    const saved = localStorage.getItem('iplinux_todo_projects');
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
  return [];
};
...
const Todo: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('iplinux_todos', JSON.stringify(tasks));
  }, [tasks]);

  useEffect(() => {
    localStorage.setItem('iplinux_todo_projects', JSON.stringify(customProjects));
  }, [customProjects]);
